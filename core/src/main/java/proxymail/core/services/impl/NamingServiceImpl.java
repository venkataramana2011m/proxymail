...
import proxymail.core.services.*;
...
public class NamingServiceImpl implements NamingService{
	...
	/*
	 * 
	 * Name Constant Provides a consistent way to access the Felix's console
	 * Author's name property also specifies, the default value of the variable
	 */
	@Property(label = "Name of the Author", value = "author")
	private static final String AUTHOR_NAME = "author.value";
	/*
	 * 
	 * Age Constant Provides a consistent way to access the Felix's console Author's
	 * age property also specifies, the default value of the variable as integer
	 * value
	 */
	...
	/*
	 * Gender Constant with which to access the Felix's console Author's Gender
	 * property also specified, default value of the variable as well as values for
	 * options i.e. DropDown for Felix's console Property
	 */
	@Property(label = "Author Gender", description = "Describe Author Gender", options = {
			@PropertyOption(name = "Male", value = "1. Male"),
			@PropertyOption(name = "Female", value = "2. Female") }, value = "Female")
	...
	@Property(value = { "English",
			"Hindi" }, unbounded = PropertyUnbounded.ARRAY, label = "Subjects", cardinality = 50, description = "Example for Multi field config")
	...
	/**
	 * @param context assigns the values to variables when the values are modified
	 *                in Felix's console
	 */
	@Modified
	protected void modified(ComponentContext context) {
		// gets values from context and assigns to instance variables
		this.name = PropertiesUtil.toString(context.getProperties().get(AUTHOR_NAME), "");
		this.gender = PropertiesUtil.toString(context.getProperties().get(AUTHOR_GENDER), "");
		this.checkbox = PropertiesUtil.toBoolean(context.getProperties().get(CHECKBOX_PROPERTY), true);
		this.multiString = PropertiesUtil.toStringArray(context.getProperties().get(MULTI_FIELD));
		this.age = PropertiesUtil.toInteger(context.getProperties().get(AUTHOR_AGE), 12);
	}
